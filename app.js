...
const baseTemplates = [
  {
    ...
    name: 'Buyer',
    tasks: [
      { id: 'buyer-consult', title: 'Buyer consultation', offsetDays: 0, defaultAnchor: 'base_date' },
      { id: 'buyer-preapproval', title: 'Confirm pre-approval letter', offsetDays: 2, defaultAnchor: 'base_date' }
    ]
  },
  {
    id: 'seller',
    name: 'Seller',
    tasks: [
      { id: 'seller-consult', title: 'Seller consultation', offsetDays: 0, defaultAnchor: 'base_date' },
      { id: 'seller-disclosures', title: 'Draft disclosures', offsetDays: 3, defaultAnchor: 'base_date' }
    ]
  },
  {
    id: 'prospect',
    name: 'Prospect',
    tasks: [
      { id: 'prospect-intro', title: 'Intro call', offsetDays: 0, defaultAnchor: 'base_date' },
      { id: 'prospect-needs', title: 'Needs analysis', offsetDays: 2, defaultAnchor: 'base_date' }
    ]
  }
];
...
function toDueDate(anchor, offsetDays) {
...
function buildTask(task, source, anchorOverride) {
  ...
  const anchorDate = state.transaction.anchors[anchorType] || null;
  return {
    ...
    anchorType,
    dueDate: toDueDate(anchorDate, task.offsetDays),
    source,
    completed: false
  };
}
...
function createTransaction(name, baseTemplateId) {
  ...
  const tx = {
    ...
    baseTemplateId,
    anchors: {
      base_date: toDateInput(new Date()),
      list_date: null,
      contract_date: null,
      closing_date: null
    },
...
function recalcTasksForAnchor(anchorType) {
  for (const task of state.transaction.tasks) {
    ...
    const date = state.transaction.anchors[anchorType];
    task.dueDate = toDueDate(date, task.offsetDays);
  }
}
    ...
    .map((task) => {
      ...
      return `
        <tr>
          ...
          <td>${task.title}</td>
          <td>${anchorLabels[task.anchorType]}</td>
          <td>${task.offsetDays > 0 ? '+' : ''}${task.offsetDays}d</td>
          <td class="${dueClass}">${task.dueDate || 'Awaiting anchor date'}</td>
          <td>${task.source}</td>
        </tr>
      `;
    })
